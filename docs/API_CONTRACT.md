...
## 5. Concurrency Note
`POST /watches` uses a SQLite `BEGIN IMMEDIATE` write transaction for the
duplicate-check, max-watch check, and insert path. This serializes competing
writes across concurrent API processes that share the same SQLite database file.

Duplicate prevention is also enforced with a unique index on
(`user_id`, `hotel_code`, `checkin_date`, `checkout_date`).
